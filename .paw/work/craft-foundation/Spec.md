...
- FR-005: Validate pinfile structure (required fields, correct types) and consistency with manifest dependencies (each manifest dependency has a pinfile entry and vice versa) (Stories: P1-Validate)
...
- FR-011: Validate dependency URL format (host/org/repo@version pattern) without network access (Stories: P1-Validate)
...
- FR-014: Version string validation — must conform to semantic versioning format (MAJOR.MINOR.PATCH) (Stories: P1-Init, P1-Validate)
- FR-015: `craft init` skill directory auto-discovery — recursively find directories containing SKILL.md files and suggest them for the skills array (Stories: P1-Init)
...
- Dependency URLs follow the format `host/org/repo@version` (e.g., `github.com/example/skills@v1.0.0`)
- Version strings follow semantic versioning: `MAJOR.MINOR.PATCH` (e.g., `1.0.0`, `0.1.0`)
